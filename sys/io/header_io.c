...
int io_read_header(dev_hook *hook, dc_header **header, xts_key **out_key, dc_pass *password)
{
	...
	if ( (*header = mm_alloc(hdr_len, MEM_SECURE | MEM_SUCCESS)) == NULL ) {
		return ST_NOMEM;
	}
	do
	{
		/* read volume header */
		if ( (resl = io_hook_rw(hook, *header, hdr_len, 0, 1)) != ST_OK ) {
			break;
		}
...
int io_write_header(dev_hook *hook, dc_header *header, xts_key *hdr_key, dc_pass *password)
{
	...
	do
	{
		if ( (hcopy = mm_alloc(hdr_len, MEM_SECURE | MEM_SUCCESS)) == NULL ) {
			resl = ST_NOMEM; break;
		} else {
			autocpy(hcopy, header, sizeof(dc_header));
		}
		if (h_key == NULL)
		{
			if ( (h_key = mm_alloc(sizeof(xts_key), MEM_SECURE | MEM_SUCCESS)) == NULL ) {
				resl = ST_NOMEM; break;
			}
		}
		if (hdr_key == NULL)
		{
			...
			*/
			...
			/* copy salt to header */
			autocpy(hcopy->salt, salt, PKCS5_SALT_SIZE);
			/* init new header key */
